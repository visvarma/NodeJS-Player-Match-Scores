...
const formattingObject = (dbObject) => {
  return {
...
app.get("/players/", async (req, res) => {
  try {
    ...
    res.send(
      dbObject.map((player) => {
        return formattingObject(player);
      })
    );
  } catch (e) {
...
app.put("/players/:playerId", async (req, res) => {
  try {
    ...
    SET player_name='${req.body.playerName}'
    WHERE player_id==${req.params.playerId}`;
...
app.get("/matches/:matchId/players", async (request, response) => {
  ...
  response.send(playersArray.map((eachPlayer) => formattingObject(eachPlayer)));
});
